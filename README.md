...
- **Watchlist** tracks players by ID, online or offline.
  - Each entry leads with the **nickname you chose** and shows the live in-game name
    beneath it. Rename inline with the button, or **double-click the nickname**; the
    nickname sticks and never triggers a lookup.
  - **Role** is a dropdown right in the row (Admin, Suspect, Player, Other): change it
    without re-adding anyone. **Ctrl+click** several rows to select them, then change
    any one of their roles to set them all at once.
